...
export default async function handler(req, res) {
 ...
 3. 학생이 질문이나 대화에서 반말(존댓말이 아닌 어조)을 사용하는 경우에 대해 다음 규칙을 적용해.
    ...
    - **이번이 2번째 이상 반말인 경우 (현재 banmalCount가 1 이상인 상태에서 반말이 감지된 경우)**:
      반드시 답변 맨 첫머리에 정확히 \`[BANMAL_DETECTED: 2]\` 태그를 붙이고, 다정하게 높임말 사용을 다시 권장해.
      안내 문구: "존댓말을 사용해 달라고 말씀드렸는데 다시 반말을 사용하셨네요. 우리 서로 존중하며 높임말로 대화해요. 다음부터는 꼭 높임말로 질문해 주세요! 😊"
      이 권장 문구 뒤에, 학생이 질문한 코딩 내용에 대한 답변을 정중한 높임말로 작성해줘.
